...
def thread_table_list(project_name, search_key):
    table_list = []
...
def insert_bq(data,table):
    ...
    result = table_insert.result()
    if (table_insert.errors is not None):
        status = 'Error: Inserting into ' + table


def update_bq(job_id):
    global update_error, status
    update_error = False
    table = cfg.status_table    
    bq_client = bigquery.Client(credentials=credentials_data,project=cfg.fnl_prj)
    Query = 'UPDATE ' + table + ' SET status = "' + status + '" WHERE job_id = "' + job_id + '"'
    table_update = bq_client.query(Query)
    result = table_update.result()
    if ((table_update.errors is not None) or (table_update.num_dml_affected_rows == 0)):
...
@app.route('/')
def main():
    global status, job_id, update_error
    status = ''
    args = request.args
